...
VoiceAppListModel::VoiceAppListModel(QObject *parent)
...
QHash<int, QByteArray> VoiceAppListModel::roleNames() const
{
...
void VoiceAppListModel::sycocaDbChanged(const QStringList &changes)
{
...
bool appNameLessThan(const VapplicationData &a1, const VapplicationData &a2)
{
    return a1.name.toLower() < a2.name.toLower();
}

void VoiceAppListModel::loadApplications()
{
    ...
    QMap<int, VapplicationData> orderedList;
    QList<VapplicationData> unorderedList;
        ...
        if (groupEntry->isType(KST_KServiceGroup)) {
            ...
            if (!serviceGroup->noDisplay()) {
                ...
                for(KServiceGroup::List::ConstIterator it = entryGroupList.constBegin();  it != entryGroupList.constEnd(); it++) {
                    ...
                    } else if (entry->property("Categories") == "VoiceApp") {
...
QVariant VoiceAppListModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid()) {
        return QVariant();
    }
...
Qt::ItemFlags VoiceAppListModel::flags(const QModelIndex &index) const
{
    if (!index.isValid())
        return nullptr;
...
int VoiceAppListModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid()) {
        return 0;
    }
...
void VoiceAppListModel::moveRow(const QModelIndex& /* sourceParent */, int sourceRow, const QModelIndex& /* destinationParent */, int destinationChild)
{
    moveItem(sourceRow, destinationChild);
}

Q_INVOKABLE void VoiceAppListModel::moveItem(int row, int destination)
{
    ...
    if (destination > row) {
        VapplicationData data = m_applicationList.at(row);
        m_applicationList.insert(destination, data);
        m_applicationList.takeAt(row);
    } else {
        VapplicationData data = m_applicationList.takeAt(row);
        m_applicationList.insert(destination, data);
    }
...
void VoiceAppListModel::runApplication(const QString &storageId)
{
...
QStringList VoiceAppListModel::appOrder() const
{
    return m_appOrder;
}

void VoiceAppListModel::setAppOrder(const QStringList &order)
{
    if (m_appOrder == order) {
        return;
    }
